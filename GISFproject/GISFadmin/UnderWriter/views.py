...
from .models import XxgenInsuredAddress
from .forms import InsuredAddressForm, InsuredDtlsForm
...
from .forms import InsuredDtlsForm, InsuredAddressForm, PolicyDtlsForm, VehicleDtlsForm, PolicyRiskDtlsForm
from .models import XxgenPolicyBills, XxgenInsuredDtls, XxgenInsuredAddress, \
    XxgenPolicyVehicleDtls, XxgenPolicyRiskDtls, XxgenPolicyDtls, XxgenPolicyBills, XxgenPolicyAgentComm
    ...
    if request.method == "GET":
        ...
        if request.GET.get('Sid') != "None" \
            ...
            try:
                ...
                context['Policyform'] = PolicyForm
                context['PolicyRishForm'] = formset
                context['message'] = "error while getting record from XxgenPolicyDtls Table"
                return render(request, 'UW/Policy_Update.html', context)
        ...
        if PolicyForm.is_valid():
            ...
            if formset.is_valid():
                for risk in formset:
                    ...
                    if not data.risk_code is None:
                        l_risk_year_prem = 0
                        if not XxgenPolicyRiskDtls.objects.filter(policy_no=data.policy_no,
                                                                  risk_code=data.risk_code).exists():
                            vobj = XxgenPolicyVehicleDtls.objects.get(policy_no=data.policy_no)
                            data.risk_description, data.risk_SA, l_risk_year_prem = \
                                motor_Risk_prem_calc(data.risk_code, vobj.vehicle_year, vobj.vehicle_make,
                                                     vobj.vehicle_model_id, vobj.vehicle_model_name)
                            data.risk_premium = ProRate_prem_calc(l_risk_year_prem, policy.end_date)
                            print('data.risk_premium', data.risk_premium)
                            data.policy_no = policy
                            data.start_Date = policy_start_Date
                            data.end_Date = policy_end_date
                            l_tot_prem = l_tot_prem + data.risk_premium
                            data.created_by = str(request.user)
                            data.last_updated_by = str(request.user)
                            data.save()
                ...
                policy.total_premium = l_tot_prem
                print('policy.total_premium',policy.total_premium)
                policy.save()
